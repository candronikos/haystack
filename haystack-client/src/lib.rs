...
async fn new_hs_session<'a>(
    uri: String,
    username: String,
    password: String,
    accept_invalid_certs: bool,
    existing_session: Arc<Mutex<Option<String>>>,
    buffer: Option<usize>,
) -> Result<(AbortHandle, mpsc::Sender<HaystackOpTxRx>, Option<String>), Error<'a>> {
    ...
    http_client = http_client.danger_accept_invalid_certs(accept_invalid_certs);
                    ...
                    match HSession::_request(ctx, op).await {
                        Ok((op, res)) => {
                            ...
                            if let Err(e) = op.resp_tx.send(HaystackResponse::Raw(res.to_string()))
                            {
                                ...
                                eprintln!(
                                    "Handling failed requests to channel not supported! {}",
                                    e
                                );
                            };
                            //Ok(())
...
impl<'a> HSession {
    pub async fn new(
        ...
        password: String,
        accept_invalid_certs: bool,
        existing_session: Arc<tokio::sync::Mutex<Option<String>>>,
        buffer: Option<usize>,
    ) -> Result<(AbortHandle, mpsc::Sender<HaystackOpTxRx>, Option<String>), Error<'a>> {
    ...
    async fn _request(
        ctx: HTTPContext,
        haystack_op: HaystackOpTxRx,
    ) -> AnyResult<(HaystackOpTxRx, FStr<'a>)> {
        let (method, op, body) = (
            haystack_op.priv_method(),
            haystack_op.priv_op(),
            haystack_op.priv_body(),
        );
        ...
        let auth = {
            ...
            auth_clone
                .to_owned()
                .ok_or_else(|| anyhow!("No auth method in haystack session object"))?
        };
        ...
        let req = ctx
            ...
            .request(
                reqwest::Method::from_str(method.as_str())
                    .map_err(|_| anyhow!("MSG: Invalid method"))?,
                ctx.uri
                    .clone()
                    .join(op.as_str())
        ...
        let req = match method.as_str() {
            "PUT" | "POST" | "PATCH" => req.body(reqwest::Body::from(
                body.ok_or(anyhow!(
                    "Request body not provided for POST, PUT or PATCH request"
                ))?
                .to_string(),
            )),
            _ => req,
    ...
    async fn _authenticate(&mut self) -> AnyResult<String> {
        ...
        .or_else(|e| {
            Err(anyhow!(
                "Auth Error: Unable to format HELLO msg: \"{:?}\"",
                e
            ))
        })?;
        ...
        let res = client
            ...
            .send()
            .await
            .or_else(|e| Err(anyhow!("Auth Error: {:?}", e)))?;

        let www_auth_header = res.headers().get("www-authenticate").ok_or(anyhow!(
            "Server response missing \"www-authenticate\" 1:\nHEADERS: {:?}",
            res.headers()
        ))?;

        let input = www_auth_header
            .to_str()
        ...
        let state = scram::ScramClient::new(self.username.as_str(), self.password.as_str(), None);
        ...
        let www_auth_header = res.headers().get("www-authenticate").ok_or(anyhow!(
            "Server response missing \"www-authenticate\" 2:\nHEADERS: {:?}",
            res.headers()
        ))?;
        ...
        let state = state
            .handle_server_first(data_temp_1)
        ...
        let res = client
            .get(
                self.uri
                    .clone()
                    .join("about")
                    .map_err(|e| anyhow!("URI: {:?}", e))?,
            )
            .header("Authorization", data.as_str())
            .send()
            .await
            .map_err(|e| anyhow!("RQW: {:?}", e))?;

        let authentication_info = (&res.headers()).get("authentication-info").ok_or(anyhow!(
            "Server response missing \"authentication-info\": HEADERS: {:?}\nSTATUS: {:?}",
            res.headers(),
            res.status()
        ))?;
        ...
        let authentication_info_map: HashMap<String, String> =
            authentication_info_list.into_iter().collect();

        let data_temp = authentication_info_map
            .get("data")
            .ok_or(anyhow!("\"data\" missing from server response"))?;
        ...
        let () = state
            .handle_server_final(data_temp_1)
            .map_err(|e| anyhow!("SCRAM state: {:?}", e))?;
...
mod tests {
    ...
    async fn client() -> Box<mpsc::Sender<ops::HaystackOpTxRx>> {
        let hs_session: (
            AbortHandle,
            mpsc::Sender<ops::HaystackOpTxRx>,
            Option<String>,
        ) = HSession::new(
    ...
    {
        let (op, resp) =
            HaystackOpTxRx::his_read("@p:demo:r:26464231-bea9f430", "2019-01-01").unwrap();
