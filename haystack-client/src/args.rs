...
fn cmd_read(op: &OP) -> Command {
    ...
    cmd = cmd
        ...
        .group(ArgGroup::new("read").args(["filter", "ids"]).required(true));
    cmd
}
...
pub fn match_filetypes(
    matches: &ArgMatches,
) -> AnyResult<(HaystackOpTxRx, Receiver<HaystackResponse>)> {
    let filter = matches
        .get_one::<String>("filter")
        .map(|s| FStr::Str(s.as_str()));
    let limit = matches.get_one::<usize>("limit").map(|v| *v);
...
fn match_his_read(sub_m: &ArgMatches) -> AnyResult<(HaystackOpTxRx, Receiver<HaystackResponse>)> {
    ...
    let ids = match sub_m.get_many::<String>("ids") {
        ...
        None => match IsTTY::new().stdin {
            true => Err(anyhow::anyhow!("hisRead op must have ids"))?,
            false => {
                ...
                stdin_ids = String::from_utf8(buf).map_err(|e| {
                    anyhow::anyhow!("Failed to convert stdin to UTF-8 string: {}", e)
                })?;
    ...
    match id_count {
        0 => Err(anyhow::anyhow!("hisRead op must have ids"))?,
        1 => {
            let id = ids
                .first()
                .ok_or_else(|| anyhow::anyhow!("Failed to get id"))?;
            HaystackOpTxRx::his_read(id.as_str(), range)
                .or_else(|e| Err(anyhow::anyhow!("Failed to create hisRead op: {:?}", e)))
        }
        _ => {
...
fn match_his_write(
    sub_m: &ArgMatches,
) -> Result<(HaystackOpTxRx, Receiver<HaystackResponse>), Error> {
    let his = sub_m
        .get_one::<String>("data")
        .ok_or_else(|| anyhow::anyhow!("His data not provided"))?;
    HaystackOpTxRx::his_write(his.as_str())
        .or_else(|e| Err(anyhow::anyhow!("Failed to create hisWrite op: {:?}", e)))
}

fn match_nav(sub_m: &ArgMatches) -> Result<(HaystackOpTxRx, Receiver<HaystackResponse>), Error> {
    let nav_id = sub_m.get_one::<String>("nav").map(|s| s.as_str());
    HaystackOpTxRx::nav(nav_id)
        .or_else(|e| Err(anyhow::anyhow!("Failed to create nav op: {:?}", e)))
}
...
fn match_read(sub_m: &ArgMatches) -> Result<(HaystackOpTxRx, Receiver<HaystackResponse>), Error> {
    if let Some(filter) = sub_m.get_one::<String>("filter") {
        HaystackOpTxRx::read(
            FStr::Str(filter.as_str()),
            sub_m.get_one::<usize>("limit").map(|v| *v),
        )
        .or_else(|e| Err(anyhow::anyhow!("Failed to create read op: {:?}", e)))
...
fn match_watch_poll(
    ...
    let watch_id = sub_m
        .get_one::<String>("watchId")
        .map(|s| s.as_str())
        .ok_or_else(|| anyhow::anyhow!("watchId not provided"))?;
    let refresh = sub_m.get_one::<bool>("refresh").map_or(false, |x| *x);
    HaystackOpTxRx::watch_poll(watch_id, refresh)
        .or_else(|e| Err(anyhow::anyhow!("Failed to create watchPoll op: {:?}", e)))
}
...
fn match_watch_sub(
    ...
    HaystackOpTxRx::watch_sub(
        watch_dis,
        watch_id,
        lease,
        ids.map(|vr| vr.map(|s| s.as_str())),
    )
    .or_else(|e| Err(anyhow::anyhow!("Failed to create watchPoll op: {:?}", e)))
}

fn match_watch_unsub(
    sub_m: &ArgMatches,
) -> Result<(HaystackOpTxRx, Receiver<HaystackResponse>), Error> {
    let watch_id = sub_m
        .get_one::<String>("watchId")
    ...
    HaystackOpTxRx::watch_unsub(watch_id, ids.map(|vr| vr.map(|s| s.as_str())), close)
        .or_else(|e| Err(anyhow::anyhow!("Failed to create watchUnsub op: {:?}", e)))
}
...
async fn repl_generic<'a, T: NumTrait>(
    ...
    let (op, resp) = m_func(&matches)
        .or_else(|e| Err(anyhow::anyhow!("Failed to create haystack op: {:?}", e)))?;

    let response = send_haystack_op::<T>(client, resp, op)
        .await
        .or_else(|e| Err(anyhow::anyhow!("Failed to get response: {:?}", e)))?;
...
async fn repl_auth<'a, T: NumTrait + 'a>(
    matches: ArgMatches,
    context: &mut Context<'a>,
) -> AnyResult<Option<String>> {
...
async fn repl_about<'a, T: NumTrait + 'a>(
    matches: ArgMatches,
    context: &mut Context<'a>,
) -> AnyResult<Option<String>> {
...
async fn repl_ops<'a, T: NumTrait + 'a>(
    matches: ArgMatches,
    context: &mut Context<'a>,
) -> AnyResult<Option<String>> {
...
async fn repl_filetypes<'a, T: NumTrait + 'a>(
    matches: ArgMatches,
    context: &mut Context<'a>,
) -> AnyResult<Option<String>> {
...
async fn repl_nav<'a, T: NumTrait + 'a>(
    matches: ArgMatches,
    context: &mut Context<'a>,
) -> AnyResult<Option<String>> {
...
async fn repl_defs<'a, T: NumTrait + 'a>(
    matches: ArgMatches,
    context: &mut Context<'a>,
) -> AnyResult<Option<String>> {
...
async fn repl_libs<'a, T: NumTrait + 'a>(
    matches: ArgMatches,
    context: &mut Context<'a>,
) -> AnyResult<Option<String>> {
...
async fn repl_read<'a, T: NumTrait + 'a>(
    matches: ArgMatches,
    context: &mut Context<'a>,
) -> AnyResult<Option<String>> {
...
async fn repl_his_read<'a, T: NumTrait + 'a>(
    matches: ArgMatches,
    context: &mut Context<'a>,
) -> AnyResult<Option<String>> {
...
async fn repl_watch_sub<'a, T: NumTrait + 'a>(
    matches: ArgMatches,
    context: &mut Context<'a>,
) -> AnyResult<Option<String>> {
...
async fn repl_watch_unsub<'a, T: NumTrait + 'a>(
    matches: ArgMatches,
    context: &mut Context<'a>,
) -> AnyResult<Option<String>> {
...
async fn repl_watch_poll<'a, T: NumTrait + 'a>(
    matches: ArgMatches,
    context: &mut Context<'a>,
) -> AnyResult<Option<String>> {
...
async fn repl_watch_his_write<'a, T: NumTrait + 'a>(
    matches: ArgMatches,
    context: &mut Context<'a>,
) -> AnyResult<Option<String>> {
...
pub fn repl<'a, T: NumTrait + 'a>(
    client: &'a mut Sender<HaystackOpTxRx>,
    abort_handle: &'a AbortHandle,
    history_file: PathBuf,
) -> Repl<Context<'a>, anyhow::Error> {
    ...
    repl_obj = repl_obj
        .with_command_async(get_cmd("op:about"), |args, context| {
            Box::pin(repl_auth::<T>(args, context))
        })
        .with_command_async(get_cmd("op:about"), |args, context| {
            Box::pin(repl_about::<T>(args, context))
        })
        .with_command_async(get_cmd("op:backup"), |args, context| {
            Box::pin(repl_not_implemented(args, context))
        })
        .with_command_async(get_cmd("op:commit"), |args, context| {
            Box::pin(repl_not_implemented(args, context))
        })
        .with_command_async(get_cmd("op:defs"), |args, context| {
            Box::pin(repl_defs::<T>(args, context))
        })
        .with_command_async(get_cmd("op:eval"), |args, context| {
            Box::pin(repl_not_implemented(args, context))
        })
        .with_command_async(get_cmd("op:evalAll"), |args, context| {
            Box::pin(repl_not_implemented(args, context))
        })
        .with_command_async(get_cmd("op:export"), |args, context| {
            Box::pin(repl_not_implemented(args, context))
        })
        .with_command_async(get_cmd("op:ext"), |args, context| {
            Box::pin(repl_not_implemented(args, context))
        })
        .with_command_async(get_cmd("op:file"), |args, context| {
            Box::pin(repl_not_implemented(args, context))
        })
        .with_command_async(get_cmd("op:filetypes"), |args, context| {
            Box::pin(repl_filetypes::<T>(args, context))
        })
        .with_command_async(get_cmd("op:funcShim"), |args, context| {
            Box::pin(repl_not_implemented(args, context))
        })
        .with_command_async(get_cmd("op:hisRead"), |args, context| {
            Box::pin(repl_his_read::<T>(args, context))
        })
        .with_command_async(get_cmd("op:hisWrite"), |args, context| {
            Box::pin(repl_watch_his_write::<T>(args, context))
        })
        .with_command_async(get_cmd("op:invokeAction"), |args, context| {
            Box::pin(repl_not_implemented(args, context))
        })
        .with_command_async(get_cmd("op:io"), |args, context| {
            Box::pin(repl_not_implemented(args, context))
        })
        .with_command_async(get_cmd("op:libs"), |args, context| {
            Box::pin(repl_libs::<T>(args, context))
        })
        .with_command_async(get_cmd("op:link"), |args, context| {
            Box::pin(repl_not_implemented(args, context))
        })
        .with_command_async(get_cmd("op:nav"), |args, context| {
            Box::pin(repl_nav::<T>(args, context))
        })
        .with_command_async(get_cmd("op:ops"), |args, context| {
            Box::pin(repl_ops::<T>(args, context))
        })
        .with_command_async(get_cmd("op:pointWrite"), |args, context| {
            Box::pin(repl_not_implemented(args, context))
        })
        .with_command_async(get_cmd("op:read"), |args, context| {
            Box::pin(repl_read::<T>(args, context))
        })
        .with_command_async(get_cmd("op:rec"), |args, context| {
            Box::pin(repl_not_implemented(args, context))
        })
        .with_command_async(get_cmd("op:upload"), |args, context| {
            Box::pin(repl_not_implemented(args, context))
        })
        .with_command_async(get_cmd("op:watchPoll"), |args, context| {
            Box::pin(repl_watch_poll::<T>(args, context))
        })
        .with_command_async(get_cmd("op:watchSub"), |args, context| {
            Box::pin(repl_watch_sub::<T>(args, context))
        })
        .with_command_async(get_cmd("op:watchUnsub"), |args, context| {
            Box::pin(repl_watch_unsub::<T>(args, context))
        });
    repl_obj
}
...
pub async fn send_haystack_op<T: NumTrait>(
    client: &Sender<HaystackOpTxRx>,
    resp: Receiver<HaystackResponse>,
    op: HaystackOpTxRx,
) -> AnyResult<HaystackResponse> {
    client
        .send(op)
        .await
        .or_else(|e| Err(anyhow::anyhow!("Failed to send request: {:?}", e)))?;
    ...
    let res = match (cmd, matches) {
        ...
        _ => {
            return Err(anyhow::anyhow!(
                "Subcommand \"{}\" either not supported or doesn't exist",
                matches
                    .subcommand()
